...
/**
 * TextWatcher Class for Card Expiry's EditText in Upgrade Screen
 *
 * Created by Anwar Almojarkesh
 *
 */
public class EMGTextFieldCVCWatcher implements TextWatcher {
	boolean m_isProcessed;
	boolean m_isBackspace;
	EditText m_txtSelf;

	public EMGTextFieldCVCWatcher() {
		m_isProcessed = false;
		m_isBackspace = true;
	}

	public EMGTextFieldCVCWatcher(EditText txt) {
		m_txtSelf = txt;
		m_isProcessed = false;
		m_isBackspace = true;
	}
	...
	private String getFormattedString(String digits, boolean p_isBackspace) {
		String szResult = "";
		szResult += digits.substring(0, Math.min(3, digits.length()));
